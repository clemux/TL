...
\part{Langages}
% * <clemux@gmail.com> 2015-01-19T14:45:35.237Z:
%
%  Fait une partie pour pas avoir 0.1 etc.
%  Use backslashes, n00b
%
...
Soit $R$ une relation d'équivalence $A \times A$ ($A^{2}) et soit $a \in A$.

\begin{itemize}
\item $[a]_{R} = { b \in A | (a,b) \in R} }$ est la classe de A relative à la relation d'équivalence R
...
\begin{itemize}
\item Soit $R$ une relation d'équivalence par $A \times A$. Alors $A/R$ est une partition de l'ensemble $A$.
\item Réciproquement si $\Pi$ est une partition de $A$ alors il existe 
une et une seule relation d'équivalence $R_{\Pi}$ tel que $\Pi = A/R_{\Pi}$.

\end{itemize}

\paragraph{Preuve :}
\begin{itemize}
\item $\forall a\in A, a\in [a]_R$ car $R$ est réflexive. Donc $A = \bigcup_{a\in A}[a]_R$
\item Soient $a, a\in A$. Montrons que si $[a]_R\cap [a']_R =\emptyset$, alors $[a]_R=[a']_R$
\end{itemize}
